...
public class DeclarationTools {
	...
	public static void addDeclarations(IDeclarationBlock target, IDeclarationBlock source) {
		for (IDeclarationElement element : source.getDeclarationElements()) {
	...
	public static Object getDeclarationDefaultValue(IDeclarationBlock declarationBlock, String path, IArgs scope) {
		ArgumentDeclaration declaration = (ArgumentDeclaration) getDeclarationElement(declarationBlock, path);
		if (declaration == null) {
			// no such declaration
			return null;
		}
	...
	public static boolean getDeclarationDefaultValueAsBoolean(IDeclarationBlock declarationBlock, String path,
			IArgs scope) {
	...
	public static String getDeclarationDefaultValueAsString(IDeclarationBlock declarationBlock, String path,
			IArgs scope) {
		Object value = DeclarationTools.getDeclarationDefaultValue(declarationBlock, path, scope);
		if (value == null) {
			return null;
	...
	public static IDeclarationElement getDeclarationElement(IDeclarationBlock declarationBlock, String path) {
		...
		for (int i = 0; i < segments.length; i++) {
			...
			if (result instanceof IDeclarationBlock) {
				...
				currentBlock = ((IDeclarationSupport) result).getDeclarationBlock();
			} else {
