...
public class ArgsValidator extends ArgsCryptoBase {
	...
	private List<Certificate> loadCertificates(IArgs contentArgs) throws ArgsValidationException {
		...
		IArgs certificatesArgs = ArgTools.getArgs(contentArgs, ARG_SIGNER + "." + ARG_CERTIFICATES, Args.create());
		for (int i = 0; i < certificatesArgs.size(); i++) {
			...
			String certType = ArgTools.getString(certArgs, ARG_TYPE, DEFAULT_CERTIFICATE_TYPE);
			try {
				Certificate cert = CertificateFactory.getInstance(certType)
						.generateCertificate(new ByteArrayInputStream(certValue));
	...
	public List<Certificate> validate(IArgs args, IArgs signatureArgs) throws ArgsValidationException {
		...
		String version = ArgTools.getString(signatureArgs, ARG_VERSION, VERSION_1_0);
		if (TYPE_ARGDSIG.equals(type) && VERSION_1_0.equals(version)) {
			List<Certificate> certificates = validateSignature(signatureArgs);
	...
	private List<Certificate> validateSignature(IArgs signatureArgs) throws ArgsValidationException {
		IArgs contentArgs = ArgTools.getArgs(signatureArgs, ARG_CONTENT, Args.create());
		byte[] signatureValue = ArgTools.getByteArray(contentArgs, ARG_VALUE, null);
		if (signatureValue == null) {
			throw new ArgsValidationException("signature.content.value is missing");
		}
		IArgs signedArgs = ArgTools.getArgs(contentArgs, ARG_SIGNED, Args.create());
		String signatureAlgorithm = ArgTools.getString(signedArgs, ARG_ALGORITHM, DEFAULT_SIGNATURE_ALGORITHM);
	...
	private void validateSignedArgsHash(IArgs args, IArgs signatureArgs) throws ArgsValidationException {
		IArgs signedArgs = ArgTools.getArgs(signatureArgs, ARG_CONTENT + "." + ARG_SIGNED, Args.create());
		byte[] hash = ArgTools.getByteArray(signedArgs, ARG_HASH + "." + ARG_RAW, null);
		if (hash == null) {
			// deprecated style
			hash = ArgTools.getByteArray(signedArgs, ARG_HASH + "." + ARG_VALUE, null);
		}
		if (hash == null) {
			throw new ArgsValidationException("signature.content.signed.hash.raw is missing");
		}

		List<String> fields = ArgTools.getList(signedArgs, ARG_SELECT, DEFAULT_SELECT);
		String digestAlgorithm = ArgTools.getString(signedArgs, ARG_HASH + "." + ARG_ALGORITHM,
