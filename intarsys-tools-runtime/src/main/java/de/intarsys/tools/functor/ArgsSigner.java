...
public class ArgsSigner extends ArgsCryptoBase {
	...
	protected IArgs createHashArgs(String digestAlgorithm, IArgs args) throws GeneralSecurityException {
		...
		hashArgs.put(ARG_RAW, new String(Base64.encode(hashValue)));
		return hashArgs;
	}
	 ...
	 * @throws GeneralSecurityException
	 */
	protected String createSignatureValue(String signatureAlgorithm, PrivateKey privateKey, IArgs args)
			throws GeneralSecurityException {
	 ...
	 * @throws GeneralSecurityException
	 */
	protected IArgs createSignedArgs(IArgs args, String signatureAlgorithm, List<String> fields, String digestAlgorithm)
			throws GeneralSecurityException {
	...
	protected IArgs createSignerArgs(List<Certificate> certificates) throws CertificateEncodingException {
		...
		for (Certificate certificate : certificates) {
			...
			certArgs.put(ARG_VALUE, new String(Base64.encode(certificate.getEncoded())));
		}
		return signerArgs;
	}
	 ...
	 * 
	 * @param args               to be signed
	 * @param fields             to filter the signed args. If empty, a default will
	 *                           be used.
	 * @param digestAlgorithm    If null, default SHA-256 will be used
	 * @param signatureAlgorithm If null, default SHA256withRSA will be used
	 * @param privateKey         a RSA private key
	 * @param certificates       matching certificate to the private key
	 * @return The {@link IArgs} structure containing the signature on args,
	 *         restricted to the subtree defined by fields.
	 * @throws GeneralSecurityException
	 */
	public IArgs sign(IArgs args, List<String> fields, String digestAlgorithm, String signatureAlgorithm,
			PrivateKey privateKey, List<Certificate> certificates) throws GeneralSecurityException {
		...
		IArgs signedArgs = createSignedArgs(args, signatureAlgorithm, fields, digestAlgorithm);
		...
		String signatureValue = createSignatureValue(signatureAlgorithm, privateKey, signedArgs);
		signatureContentArgs.put(ARG_VALUE, signatureValue);
		return signatureArgs;
	}

	/**
	 * Sign the {@link IArgs} structure with the provided private key using default
	 * algorithms.
	 * 
	 * @param args         to be signed
	 * @param privateKey   a RSA private key
	 ...
	 * @throws GeneralSecurityException
	 */
	public IArgs sign(IArgs args, PrivateKey privateKey, List<Certificate> certificates)
			throws GeneralSecurityException {
		return sign(args, Collections.<String>emptyList(), DEFAULT_DIGEST_ALGORITHM, DEFAULT_SIGNATURE_ALGORITHM,
