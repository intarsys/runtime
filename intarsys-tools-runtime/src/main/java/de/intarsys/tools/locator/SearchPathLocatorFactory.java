...
public class SearchPathLocatorFactory extends DelegatingLocatorFactory
	...
	protected void addSearchPath(File parent, String path) {
		...
		((LookupLocatorFactory) getFactory()).addLocatorFactory(new RelativeLocatorFactory(locator));
	}
	...
	public void setSearchPath(File parent, String paths) {
		...
		// set up search paths
		for (Enumeration e = new StringTokenizer(paths, PATH_SEPARATOR); e.hasMoreElements();) {
	...
	public void toString(PrettyPrinter printer) {
		printer.appendValue(getClass().getName() + " (" + searchpath + ") on " + getFactory());
	}
