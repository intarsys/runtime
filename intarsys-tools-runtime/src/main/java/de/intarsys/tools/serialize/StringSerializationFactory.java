...
public class StringSerializationFactory implements ISerializationFactory, IMimeTypeSupport {
	...
	public IDeserializer createDeserializer(SerializationContext context) {
		return new StringDeserializer(((StreamSerializationContext) context).getInputStream(), getCharset());
	}

	@Override
	public ISerializer createSerializer(SerializationContext context) {
		return new StringSerializer(((StreamSerializationContext) context).getOutputStream(), getCharset());
	}
