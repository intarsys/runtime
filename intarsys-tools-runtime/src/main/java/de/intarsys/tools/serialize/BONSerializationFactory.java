...
public class BONSerializationFactory implements ISerializationFactory, IMimeTypeSupport {
	...
	public IDeserializer createDeserializer(SerializationContext context) {
		return new BONDeserializer(((StreamSerializationContext) context).getInputStream());
	}

	@Override
	public ISerializer createSerializer(SerializationContext context) {
		return new BONSerializer(((StreamSerializationContext) context).getOutputStream());
	}
